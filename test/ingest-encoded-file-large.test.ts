...
describe("IngestEncodedFile Large Size Test", () => {
  ...
  it("should test larger file sizes from 50MB upward", async () => {
    ...
    if (maxSuccess >= 50) {
      console.log(`✅ Customers can reliably upload files up to ${maxSuccess}MB`);
      console.log(`📝 For best results, recommend keeping files under ${Math.floor(maxSuccess * 0.9)}MB`);
    } else if (maxSuccess > 0) {
      console.log(`⚠️  Maximum successful upload was ${maxSuccess}MB`);
      console.log(`📝 Recommend keeping files under ${Math.floor(maxSuccess * 0.9)}MB for reliability`);
    }
