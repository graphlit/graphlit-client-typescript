...
describe.skipIf(skipTests)("Multi-Provider Duplication Tests", () => {
  ...
  testProviders.forEach((provider) => {
    it(`should not duplicate response with ${provider.name}`, async () => {
      ...
      const specResponse = await client.createSpecification(provider.spec as Types.SpecificationInput);
          ...
          (event) => {
            ...
            if (event.type === "message_update" && event.message?.message) {
              ...
              if (delta === 0 && newLength > 0) {
                ...
                console.log(`  ⚠️  ${provider.name}: Last chunk contains entire message (${newLength} chars)`);
              }
            }
      ...
      console.log(`  ✅ ${provider.name}: Final message length: ${fullMessage.length}`);
      console.log(`  📊 ${provider.name}: Total chunks: ${messageChunks.length}`);
      ...
      if (fullMessageChunks.length > 0) {
        console.log(`  ⚠️  ${provider.name}: ${fullMessageChunks.length} chunk(s) contain the full message`);
      }
