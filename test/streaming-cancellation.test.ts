...
describe.skipIf(skipTests)("Streaming Cancellation Tests", () => {
  ...
  describe("AbortController cancellation", () => {
        ...
        .streamAgent(
          ...
          (event) => {
            ...
            if (
              ...
              events.filter((e) => e.type === "message_update").length === 2
            ) {
    ...
    it("should allow multiple streams with different abort controllers", async () => {
      ...
      const stream1Promise = client
        .streamAgent(
          ...
          (event) => {
            ...
            if (
              event.type === "message_update" &&
              event.message.message.length > 100
            ) {
              controller1.abort();
            }
          },
      ...
      expect(events2.some((e) => e.type === "conversation_completed")).toBe(
        true,
      );
    }, 30000);
  });
  ...
  describe("Error handling during cancellation", () => {
    it("should handle cancellation during tool calls", async () => {
      ...
      expect(
        events.some(
          (e) => e.type === "tool_update" && e.status === "executing",
        ),
      ).toBe(true);
