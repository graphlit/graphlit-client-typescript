...
describe("Streaming Multiple Tool Calls Test", () => {
  ...
  beforeAll(async () => {
    ...
    specification = specifications.specifications?.results?.find(
      (spec) =>
  ...
  it("should track multiple separate tool calls during streaming", async () => {
    ...
    if (!client.supportsStreaming(specification)) {
      console.log("Streaming not supported for this specification, skipping test");
      return;
    }
            ...
            results: [
              {
                ...
                snippet: "Best goals and assists from Mason Marchment's 2023-24 season with the Dallas Stars"
              },
              {
            ...
            results: [
              {
                ...
                snippet: "Connor McDavid's incredible plays from the 2023-24 NHL season"
              },
              {
                title: "McDavid's 150 Point Season Highlights",
                url: "https://youtube.com/watch?v=example4",
                snippet: "Every goal and assist from Connor McDavid's historic 150+ point season"
              }
            ]
          };
        }
    ...
    console.log("Model:", specification.openAI?.model || specification.anthropic?.model);
        ...
        switch (event.type) {
          ...
          case 'tool_update':
            ...
            if (event.status === 'completed') {
              ...
              if (event.result) {
                console.log("  Result preview:", JSON.stringify(event.result).substring(0, 200) + "...");
              }
            }
            break;
    ...
    const queries = toolCallsReceived.map(call => call.arguments.query.toLowerCase());
    ...
    const completedEvents = toolStatusEvents.filter(e => e.status === 'completed');
    ...
    console.log("not in the SDK. The SDK is correctly emitting events for both searches.");
  });
    ...
    await client.streamAgent(
      ...
      async (event: AgentStreamEvent) => {
        if (event.type === 'conversation_started') {
          conversationId = event.conversationId;
        } else if (event.type === 'tool_update' && event.status === 'executing') {
    ...
    for (let i = 0; i < toolCalls.length - 1; i++) {
      for (let j = i + 1; j < toolCalls.length; j++) {
        ...
        if (call2.startTime < call1End) {
          parallelExecutions++;
          console.log(`Parallel execution detected: ${call1.tool} and ${call2.tool}`);
        }
      }
    }
