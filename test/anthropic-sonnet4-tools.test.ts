...
describe("Anthropic Sonnet 4 - Thinking + Tool Calls Issue", () => {
  ...
  if (!orgId || !envId || !secret || !anthropicKey) {
    console.warn("⚠️  Skipping Anthropic Sonnet 4 tests - missing credentials");
    return;
  }
  ...
  it(
    "should handle thinking + tool calls without closing stream prematurely",
    { timeout: 60000 },
    async () => {
          ...
          switch (event.type) {
            ...
            case "reasoning_update":
              ...
              if (!reasoningEvent.isComplete) {
                console.log(
                  `  🤔 Thinking... (${reasoningEvent.content?.length || 0} chars)`,
                );
              } else {
                console.log(
                  `  ✅ Thinking complete (${reasoningEvent.content?.length || 0} chars)`,
                );
              }
              break;
            ...
            case "message_update":
              ...
              if (messageEvent.message?.message?.length > 0) {
                console.log(
                  `  💬 Message update (${messageEvent.message.message.length} chars, streaming: ${messageEvent.isStreaming})`,
                );
              }
              break;
            ...
            case "conversation_completed":
              ...
              const hasPendingTools =
                completedEvent.message?.toolCalls?.length > 0;
              ...
              if (hasPendingTools && !toolExecutionCompleted) {
                console.log(
                  `  ⚠️  ISSUE DETECTED: conversation_completed with pending tools!`,
                );
      ...
      const conversationCompletedIndex = events.findIndex(
        (e) => e.type === "conversation_completed",
      );
      const lastToolUpdateIndex =
        events
          .map((e, i) => (e.type === "tool_update" ? i : -1))
          .filter((i) => i >= 0)
          .pop() || -1;
      ...
      console.log(
        `  Conversation completed index: ${conversationCompletedIndex}`,
      );
      ...
      if (conversationCompletedIndex > 0 && lastToolUpdateIndex > 0) {
        if (conversationCompletedIndex < lastToolUpdateIndex) {
          console.log(
            `  ✅ GOOD: Tool updates continued after conversation_completed`,
          );
        } else {
          console.log(
            `  ⚠️  ISSUE: conversation_completed came after last tool update`,
          );
          ...
          if (lastToolEvent.details?.status !== "completed") {
            console.log(
              `  ❌ PROBLEM CONFIRMED: Stream closed with tool in '${lastToolEvent.details?.status}' status`,
            );
            streamClosedPrematurely = true;
          }
        }
  ...
  it(
    "should properly handle multiple rounds with thinking",
    { timeout: 60000 },
    async () => {
