  ...
  async function testFileSize(
    ...
    try {
      ...
      if (result.ingestEncodedFile?.id) {
        ...
        console.log(
          `  ✅ SUCCESS: Uploaded in ${(uploadTime / 1000).toFixed(1)}s`,
        );
        console.log(
          `  📊 Upload speed: ${(sizeMB / (uploadTime / 1000)).toFixed(1)} MB/s`,
        );
      ...
      if (error.graphQLErrors) {
        error.graphQLErrors.forEach((gqlError: any) => {
          ...
          if (gqlError.extensions) {
            console.log(
              `     Extensions:`,
              JSON.stringify(gqlError.extensions, null, 2),
            );
          }
        });
      }
  ...
  it("should find exact limit starting from 70MB", async () => {
    ...
    for (const [size, result] of sortedResults) {
      const timeStr = result.time
        ...
        : "N/A";
      const speedStr = result.time
        ? `${(parseInt(size) / (result.time / 1000)).toFixed(1)} MB/s`
        : "N/A";
      console.log(
        `${size.padStart(9)} | ${result.success ? "✅ Success" : "❌ Failed "} | ${timeStr.padStart(11)} | ${speedStr.padStart(11)}`,
      );
    }
    ...
    console.log(
      `📝 Recommended maximum: ${Math.floor(lastSuccessfulSize * 0.95)}MB`,
    );
