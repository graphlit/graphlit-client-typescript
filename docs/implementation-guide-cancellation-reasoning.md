...
<template>
  <div>
    <button @click="startStream" :disabled="isStreaming">Generate</button>
    <button @click="cancelStream" :disabled="!isStreaming">Cancel</button>
      ...
      {reasoning && (
        <div className="reasoning-section">
          <div className="reasoning-header">
            <h3>{isThinking ? "🤔 Thinking..." : "💭 Thought Process"}</h3>
...
async function robustStream(client: Graphlit, prompt: string) {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      console.log(
        `Stream cancelled (started: ${hasStarted}, completed: ${hasCompleted})`,
      );
    } else {
      throw error;
    }
...
function useStreamDebounce(delay: number = 300) {
  ...
  const debouncedStream = useCallback(
    async (
      client: Graphlit,
      prompt: string,
